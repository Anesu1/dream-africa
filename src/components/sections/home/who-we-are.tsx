...
export default function WhoWeAre({ whoWeAre }: { whoWeAre: HomePage["whoWeAre"] }) {
      ...
      <div className="grid gap-16 lg:grid-cols-[.9fr_1.1fr] lg:gap-24">
        <div ref={ref} className="overflow-hidden rounded-sm">
          <motion.div style={{ y }} className="relative h-[420px] w-full sm:h-[560px]">
            <Image src={whoWeAre.image} alt="Guide at the Zambezi" fill className="object-cover" sizes="(min-width: 1024px) 40vw, 100vw" />
          </motion.div>
        </div>

        <div>
          <Reveal className="mb-7 text-[11px] uppercase tracking-[0.28em] text-gold">{whoWeAre.eyebrow}</Reveal>
          <Reveal>
            <h2
          ...
          <Reveal>
            <p className="mb-14 max-w-[620px] text-lg leading-relaxed text-muted text-pretty">{whoWeAre.body}</p>
          </Reveal>

          <div className="grid grid-cols-2 gap-8 sm:grid-cols-4">
            {whoWeAre.milestones.map((m, i) => (
